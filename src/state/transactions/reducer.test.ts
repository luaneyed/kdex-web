...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.CYPRESS]).toBeTruthy()
      expect(txs[ChainId.CYPRESS]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.CYPRESS]?.['0x0']
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.CYPRESS), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.CYPRESS] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.CYPRESS }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.CYPRESS), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.CYPRESS] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
    })
  })
})
